...
class DefaultStateBasedChangeResolutionStrategy implements StateBasedChangeResolutionStrategy {
        ...
        for (var i = differences.size - 1; i >= 0; i--) {
            if (differences.get(i) instanceof ReferenceChange) {
                ...
                if (diff.reference.name == "memberEnd" && differences.exists [
                    changeKindsMatching(diff.kind, kind) && it instanceof ReferenceChange &&
    ...
    private def changeKindsMatching(DifferenceKind possiblyGeneratedKind, DifferenceKind kind) {
        if (possiblyGeneratedKind == kind) {
            return true
        }
        if (possiblyGeneratedKind == DifferenceKind.ADD && kind == DifferenceKind.MOVE) {
            return true
        }
        return false
    }
